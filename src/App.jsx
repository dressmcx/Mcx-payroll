...
// ─── STORE LOCATION  ← set your store GPS here ───────────────────────────────
const STORE_LAT    = 40.700700,-73.949853;   // your store latitude
const STORE_LNG    = 40.700700,-73.949853;  // your store longitude
const STORE_RADIUS = 200;       // metres radius allowed
...
const exportCSV = ({workers,entries,payments,reminders,from,to}) => {
  ...
  const label = from&&to?`${from}_to_${to}`:todayStr();
  ...
  workers.forEach(w=>{
    ...
    const hrs=hoursFrom(we), gross=hrs*w.rate;
    const paid=wp.reduce((s,p)=>s+Number(p.amount),0), bal=Math.max(0,gross-paid);
    ...
    (alerts.length?alerts:[null]).forEach((a,i)=>{
      rows.push([
        i===0?w.name:"", i===0?hrs.toFixed(2):"", i===0?w.rate:"", i===0?fmtMoney(gross):"",
        i===0?fmtMoney(paid):"", i===0?fmtMoney(bal):"", i===0?schedSummary(getSched(w)):"",
        i===0?methods:"", i===0?checks:"",
...
// ─── SHARED COMPONENTS ────────────────────────────────────────────────────────
const Logo=({size=56,light=false})=>(
  <svg width={size} height={size} viewBox="0 0 200 200" fill="none">
    <ellipse cx="110" cy="90" rx="75" ry="32" transform="rotate(-30 110 90)" stroke={T.gold} strokeWidth="3.5" fill="none" opacity=".75"/>
    <ellipse cx="110" cy="100" rx="55" ry="22" transform="rotate(-30 110 100)" stroke={T.gold} strokeWidth="2" fill="none" opacity=".4"/>
    <text x="28" y="135" fontFamily="Georgia,serif" fontSize="88" fontWeight="700" fill={light?"#fff":"#0a0a0a"} letterSpacing="-2">mcx</text>
  </svg>
);
...
const Spinner=()=>(
  <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:T.brand,flexDirection:"column",gap:16}}>
    <Logo size={60} light/><div style={{width:36,height:36,border:`3px solid ${T.brandDark}`,borderTop:`3px solid ${T.gold}`,borderRadius:"50%",animation:"spin 1s linear infinite"}}/>
...
// Date range picker used in Payroll tab
      ...
      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:12}}>
        {[["Today",0],["Last 7 days",6],["Last 30 days",29],["This month","month"],["All time",null]].map(([l,v])=>(
          <button key={l} onClick={()=>set(v)}
            style={{padding:"5px 12px",borderRadius:20,border:"none",cursor:"pointer",fontSize:12,fontWeight:600,background:T.border,color:T.muted}}>
            {l}
          </button>
        ))}
      </div>
      <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
        {[["From","from"],["To","to"]].map(([label,field])=>(
          <div key={field} style={{flex:1,minWidth:140}}>
            ...
            <input type="date" value={field==="from"?from:to}
              onChange={e=>onChange({from,to,...{[field]:e.target.value}})}
...
export default function App() {
    ...
    workers.forEach(w=>{
      ...
      if(h===sh&&m===sm+15&&s===0){
        if(!entries.filter(e=>e.worker_id===w.id&&isToday(e.clock_in)).some(e=>!e.clock_out)){
          ...
          setReminders(r=>[...r,{id:Date.now()+w.id,workerId:w.id,msg,ts:Date.now()}]);toast$(msg,"warning");
        }
      }
      if(h===eh&&m===em&&s===0&&entries.find(e=>e.worker_id===w.id&&isToday(e.clock_in)&&!e.clock_out)){
        ...
        setReminders(r=>[...r,{id:Date.now()+w.id+1,workerId:w.id,msg,ts:Date.now()}]);toast$(msg,"warning");
      }
    });
  },[now]);
  ...
  // date-filtered helpers for payroll tab
  const inRange=ts=>{
    const d=new Date(ts);
  ...
  const geoClockIn=async wid=>{
    ...
    if(!navigator.geolocation){toast$("Geolocation not supported on this device.","warning");return;}
    ...
    navigator.geolocation.getCurrentPosition(
      async pos=>{
        ...
        if(dist<=STORE_RADIUS){
          setGeoState({status:"ok",msg:`✅ Location verified (${Math.round(dist)}m from store)`});
          await doClock(wid);
          setTimeout(()=>setGeoState({status:"idle",msg:""}),3000);
        }else{
          setGeoState({status:"far",msg:`📍 You are ${Math.round(dist)}m away. Must be within ${STORE_RADIUS}m.`});
          toast$(`❌ Too far from store (${Math.round(dist)}m away). You must be on-site to clock in.`,"warning");
        }
      },
      ()=>{
        setGeoState({status:"denied",msg:"Location access denied. Please enable location in your browser."});
        toast$("Location access denied. Please allow location to clock in.","warning");
      },
  ...
  const handlePin=d=>{
    ...
    if(next.length===4){
      setTimeout(()=>{
        if(pinTarget==="manager"){if(next===MANAGER_PIN){setScreen("manager");setMTab("dashboard");}else setPinErr("Incorrect manager PIN");}
        else{const w=workers.find(x=>x.id===pinTarget);if(w&&next===w.pin){setActiveW(w);setScreen("worker");}else setPinErr("Incorrect PIN");}
        setPinBuf("");
      },200);
    }
  };
  ...
  const submitPay=async()=>{
    ...
    setSaving(true);
    const{error}=await sb.from("payments").insert({worker_id:payModal.workerId,amount:total,methods:payRows.filter(r=>parseFloat(r.amount)>0).map(r=>({method:r.method,amount:parseFloat(r.amount),...(r.method==="Check"&&r.checkNumber?{checkNumber:r.checkNumber}:{})})),note:payNote,paid_at:new Date().toISOString()});
    if(error)toast$("Error: "+error.message,"warning");
  ...
  const deletePay=async(pid,amount)=>{
    if(!window.confirm(`Delete this payment of ${fmtMoney(amount)}? This cannot be undone.`))return;
    setSaving(true);
  ...
  if(screen==="splash")return(
    <div style={{minHeight:"100vh",background:T.brand,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:18,opacity:splashOut?0:1,transition:"opacity .6s ease"}}>
      <style>{CSS}</style>
      <div style={{animation:"popIn .7s cubic-bezier(.34,1.56,.64,1) both"}}><Logo size={130} light/></div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,width:240}}>
        {[1,2,3,4,5,6,7,8,9,"",0,"⌫"].map((d,i)=>(
          <button key={i} onClick={()=>{if(d==="⌫"){setPinBuf(p=>p.slice(0,-1));setPinErr("");}else if(d!=="")handlePin(String(d));}} disabled={d===""}
            style={{height:64,borderRadius:12,border:"none",background:d===""?"transparent":d==="⌫"?"#f0ece4":"#fff",color:"#0a0a0a",fontSize:22,fontWeight:600,cursor:d===""?"default":"pointer",boxShadow:d===""||d==="⌫"?"none":"0 2px 10px rgba(0,0,0,.08)"}}>
            {d}
          </button>
        ))}
      </div>
    ...
    <div style={{minHeight:"100vh",background:"#f8f6f2",display:"flex",flexDirection:"column",alignItems:"center",padding:"28px 16px"}}>
      ...
      <div style={{width:"100%",maxWidth:480}}>
        <div style={{textAlign:"center",marginBottom:26}}>
          <Logo size={80}/><h1 style={{fontFamily:"Georgia,serif",fontSize:30,margin:"10px 0 4px",color:"#0a0a0a",letterSpacing:-1}}>MCX Time Clock</h1>
    ...
    return(
      <div style={{minHeight:"100vh",background:isin?"#f0fdf4":"#f8f6f2",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"28px 16px",position:"relative"}}>
        ...
        <div style={{width:"100%",maxWidth:380,textAlign:"center"}}>
          ...
          {activeW.geo_bypass&&<div style={{fontSize:12,color:T.amber,marginBottom:6}}>🏠 Remote worker — location check bypassed</div>}
            ...
            <div style={{position:"fixed",inset:0,zIndex:1000,background:"rgba(0,0,0,.85)",display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
              <div style={{background:T.surface,borderRadius:20,width:"100%",maxWidth:500,border:`1px solid ${T.gold}`,maxHeight:"92vh",overflowY:"auto"}}>
                <div style={{padding:"20px 24px",borderBottom:`1px solid ${T.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div><div style={{fontFamily:"Georgia,serif",fontSize:21,fontWeight:700}}>Record Payment</div><div style={{fontSize:13,color:T.muted,marginTop:3}}>{w.name}</div></div>
                ...
                <div style={{padding:"20px 24px"}}>
                  ...
                  <button onClick={addRow} style={{width:"100%",padding:"9px",background:"transparent",border:`1px dashed ${T.border}`,borderRadius:8,color:T.faint,cursor:"pointer",fontSize:13,marginBottom:16}}>+ Split Payment — Add Another Method</button>
                  ...
                  <div style={{marginBottom:20}}>
                    ...
                    <input type="text" value={payNote} onChange={e=>setPayNote(e.target.value)} placeholder="e.g. Weekly pay, partial, bonus…" style={inp()}/>
                  </div>
              ...
              <div style={{background:T.surface,borderRadius:14,border:`1px solid ${T.border}`,overflow:"hidden"}}>
                ...
                <div style={{overflowX:"auto"}}>
                  <table style={{width:"100%",borderCollapse:"collapse",minWidth:540}}>
                    ...
                    <tbody>
                      {workers.map((w,i)=>{const isin=ci(w.id),wh=wkHrs(w.id),e=earned$(w.id,w.rate),p=paid$(w.id),b=bal$(w.id,w.rate);return(
                        <tr key={w.id} style={{borderTop:`1px solid ${T.border}`,background:i%2===0?T.surface:T.surfaceAlt}}>
                          ...
                          <td style={{padding:"11px 14px",fontSize:11,color:w.geo_bypass?T.amber:T.green}}>{w.geo_bypass?"🏠 Remote":"📍 On-site"}</td>
          ...
          {mTab==="payroll"&&(
            <div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:10}}>
                ...
                <button onClick={()=>exportCSV({workers,entries,payments,reminders,from:dateRange.from,to:dateRange.to})}
                  style={{padding:"9px 18px",background:T.green,border:"none",borderRadius:8,color:"#fff",fontWeight:700,cursor:"pointer",fontSize:13}}>⬇ Export to Excel</button>
              </div>
                  ...
                  <div key={w.id} style={{background:T.surface,borderRadius:16,border:`1px solid ${T.border}`,overflow:"hidden",marginBottom:16}}>
                    <div style={{padding:"18px 22px",borderBottom:`1px solid ${T.border}`,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12}}>
                      <div>
                        ...
                        <div style={{fontSize:12,color:T.faint,marginTop:3}}>{wh.toFixed(2)} hrs · ${w.rate}/hr{(dateRange.from||dateRange.to)?" · filtered":""}</div>
                      </div>
                        ...
                        {[...fp].reverse().map(pay=>(
                          <div key={pay.id} style={{padding:"10px 0",borderBottom:`1px solid ${T.dark}`,display:"flex",justifyContent:"space-between",alignItems:"flex-start",fontSize:13}}>
                            ...
                            <div style={{textAlign:"right",flexShrink:0,marginLeft:16,display:"flex",flexDirection:"column",alignItems:"flex-end",gap:5}}>
                              ...
                              <button onClick={()=>deletePay(pay.id,pay.amount)}
                                style={{padding:"3px 10px",background:T.redBg,border:`1px solid ${T.red}`,borderRadius:6,color:T.red,cursor:"pointer",fontSize:11,fontWeight:700}}>
                                Delete
                              </button>
                            </div>
                          </div>
              ...
              {addingW&&(
                <div style={{background:T.surface,borderRadius:14,padding:20,border:`1px solid ${T.borderHi}`,marginBottom:16}}>
                  ...
                  <div style={{marginTop:14,marginBottom:14,padding:"12px 14px",background:T.dark,borderRadius:10,border:`1px solid ${T.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <div>
                      <div style={{fontSize:13,fontWeight:700}}>🏠 Remote Worker</div>
                      <div style={{fontSize:11,color:T.faint,marginTop:3}}>Bypass location check — can clock in from anywhere</div>
                    </div>
                    ...
                    {editWid===w.id?(
                      <div>
                        ...
                        <div style={{marginTop:12,marginBottom:12,padding:"12px 14px",background:T.dark,borderRadius:10,border:`1px solid ${T.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                          <div>
                            <div style={{fontSize:13,fontWeight:700}}>🏠 Remote Worker</div>
                            <div style={{fontSize:11,color:T.faint,marginTop:3}}>Bypass location check</div>
                          </div>
                      ...
                      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between"}}>
                        <div style={{flex:1}}>
                          ...
                          <div style={{fontSize:11,marginTop:3,color:w.geo_bypass?T.amber:T.green}}>{w.geo_bypass?"🏠 Remote — location bypass ON":"📍 On-site — location required"}</div>
                        </div>
              ...
              {manEntry&&(
                <div style={{background:T.surface,borderRadius:14,padding:20,border:`1px solid ${T.gold}`,marginBottom:18}}>
                  ...
                  <div style={{marginBottom:10}}>
                    ...
                    <select value={manEntry.workerId} onChange={e=>setManEntry(p=>({...p,workerId:e.target.value}))} style={inp()}>{workers.map(w=><option key={w.id} value={w.id}>{w.name}</option>)}</select>
                  </div>
                ...
                <div key={w.id} style={{background:T.surface,borderRadius:14,border:`1px solid ${isOpen?T.gold:T.border}`,marginBottom:12,overflow:"hidden"}}>
                  ...
                  {isOpen&&(
                    <div style={{padding:"0 20px 20px",borderTop:`1px solid ${T.border}`}}>
                      <div style={{marginTop:16}}><SchedEditor schedule={schDraft} onChange={setSchDraft}/></div>
                      <div style={{display:"flex",gap:10,marginTop:16}}>
                        <button onClick={()=>saveSchTab(w.id)} disabled={saving} style={{flex:1,padding:"10px",background:T.gold,border:"none",borderRadius:8,fontWeight:700,cursor:"pointer",color:T.dark,fontSize:14}}>{saving?"Saving…":"✓ Save Schedule"}</button>
                        <button onClick={()=>setExpandSch(null)} style={{flex:1,padding:"10px",background:T.border,border:"none",borderRadius:8,color:"#888",cursor:"pointer"}}>Cancel</button>
                      </div>
                    </div>
                  )}
                </div>
            ...
            <div>
              ...
              {reminders.length===0?(
                <div style={{background:T.surface,borderRadius:14,padding:40,border:`1px solid ${T.border}`,textAlign:"center"}}>
                  ...
                  <div style={{fontSize:13,color:T.faint,marginTop:10,opacity:.6}}>Set a date range in Payroll, then use Export to Excel for a filtered report.</div>
                </div>
              ):(
                <div style={{display:"grid",gap:10}}>
                  {reminders.slice().reverse().map(r=>(
                    <div key={r.id} style={{background:T.amberBg,borderRadius:12,padding:"14px 18px",border:"1px solid #3a2800",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <div>
                      ...
                      <button onClick={()=>setReminders(p=>p.filter(x=>x.id!==r.id))} style={{background:"none",border:"none",color:T.faint,cursor:"pointer",fontSize:18,paddingLeft:12}}>✕</button>
                    </div>
                  ))}
                </div>
              )}
